...
| G-09 | §6 Calendar | Calendar is computed (astronomical); admins cannot add custom festival entries or disable computed ones | P2 | open |
| G-10 | §28, §15 | `admin_activity` records many actions but there is no Audit Logs admin page; some actions (settings, sponsors, streams) need coverage checks | P2 | open |
